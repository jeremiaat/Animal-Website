...
export default function HeroSection() {
  ...
  const handleSearch = async () => {
    if (!query) return;
    try {
      const response = await axios.get(`http://localhost:8000/api/animals/?q=${query}`);
    ...
    <section className="relative z-10 pt-28 pb-24 text-center px-4">
      ...
      <div className="max-w-xl mx-auto mb-8 relative">
        ...
        <button
          ...
          className="absolute right-3 top-1/2 transform -translate-y-1/2 bg-teal-500 hover:bg-teal-600 text-white py-1 px-3 rounded-full"
        >
          Search
        </button>
      </div>
      ...
      {results.length > 0 && (
        <ul className="max-w-xl mx-auto mt-4 bg-[var(--card-background)] rounded-lg shadow-lg overflow-hidden text-left">
          {results.map((animal) => (
            <li
              key={animal.id}
